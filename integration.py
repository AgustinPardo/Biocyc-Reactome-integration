...
# ~ genes_uncurated_list 	= 		uncurated_file['Gene name']
# ~ uniprot_uncurated_list 	= 		uncurated_file['Entry']

a=uncurated_file[['Gene name','Entry']]
genes_uniprot_list=[]
for r in a.itertuples():
	genes=[r[1]]
	uniprot=[r[2]]
	genes_uniprot_list.append([genes,uniprot])
...
salida.write('Uncurated-gene-id'+'\t'+'Uncurated-gene-uniprot-id'+'\t'+'Uncurated-gene-synonims'+'\t'+'Biocyc-gene-sinomys'+\
...
for genes_uniprot_uncurated in genes_uniprot_list:
	print(genes_uniprot_uncurated)
	print(genes_uniprot_uncurated[0])
	print(genes_uniprot_uncurated[1])

	genes_uncurated		=		genes_uniprot_uncurated[0][0].split(' ')	
	uniprot_id			=		genes_uniprot_uncurated[1][0]	
	found				=		False
	i=0
	...
	while found == False and len(genes_uncurated)>i:
			...
			if found != False:			
				...
				salida.write(genes_uncurated[0]+'\t'+uniprot_id+'\t'+str(genes_uncurated)\
